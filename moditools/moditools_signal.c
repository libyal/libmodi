...
void (*moditools_signal_signal_handler)( moditools_signal_t ) = NULL;
             ...
             unsigned long signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( moditools_signal_signal_handler != NULL )
			{
				moditools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "moditools_signal_attach";
...
#if defined( _MSC_VER )
	moditools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	moditools_signal_signal_handler = NULL;
