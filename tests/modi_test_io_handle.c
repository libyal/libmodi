...
#include "../libmodi/libmodi_io_handle.h"

#if defined( __GNUC__ ) && !defined( LIBMODI_DLL_IMPORT )

/* Tests the libmodi_io_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int modi_test_io_handle_initialize(
     void )
{
	...
	io_handle = (libmodi_io_handle_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libmodi_io_handle_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libmodi_io_handle_initialize with memset failing
		 */
...
/* Tests the libmodi_io_handle_free function
 * Returns 1 if successful or 0 if not
 */
int modi_test_io_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	MODI_TEST_RUN(
	 "libmodi_io_handle_initialize",
	 modi_test_io_handle_initialize );
